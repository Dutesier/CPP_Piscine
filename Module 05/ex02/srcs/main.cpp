...
bool    test_checker(const Form& f, bool will_sign) {
    if (state_checker(f, will_sign) == false) {
        std::cerr << "Test \033[41;1mFAILED\033[0m: Unwarranted Form Sign Change" << std::endl;
        return (false);
    }
    std::cout << "Test for signing \033[42;1mSUCESS\033[0m" << std::endl;
    return (true);
    
}

void testPPF() {
    std::cout << std::endl << "\033[33;1m" << "---> Testing Presidential Pardon Form <---" << "\033[37m" << std::endl;
    ...
    std::cout << "\033[33;1m" << "Testing Can't Sign, Won't Sign, Can't Execute" << "\033[37m" << std::endl;
    bob_a.executeForm(ppf1);
    test_checker(ppf1, false);

    std::cout << "\033[33;1m" << "Testing Can't Sign, Will Sign, Can't Execute" << "\033[37m" << std::endl;
    bob_a.signForm(ppf2);
    bob_a.executeForm(ppf2);
    test_checker(ppf2, false);

    std::cout << "\033[33;1m" << "Testing Can Sign, Won't Sign, Can't Execute" << "\033[37m" << std::endl;
    bob_b.executeForm(ppf3);
    test_checker(ppf3, false);

    std::cout << "\033[33;1m" << "Testing Can Sign, Will Sign, Can't Execute" << "\033[37m" << std::endl;
    bob_b.signForm(ppf4);
    bob_b.executeForm(ppf4);
    test_checker(ppf4, true);

    std::cout << "\033[33;1m" << "Testing Can Sign, Won't Sign, Can Execute" << "\033[37m" << std::endl;
    bob_c.executeForm(ppf5);
    test_checker(ppf5, false);

    std::cout << "\033[33;1m" << "Testing Can Sign, Will Sign, Can Execute" << "\033[37m" << std::endl;
    bob_c.signForm(ppf6);
    bob_c.executeForm(ppf6);
    test_checker(ppf6, true);
    std::cout << std::endl << "\033[33;1m" << "---> Finished Testing <---" << "\033[37m" << std::endl;
}

void testSCF()  {
    std::cout << std::endl << "\033[33;1m" << "---> Testing Shrubbery Creation Form <---" << "\033[37m" << std::endl;
    std::string target = "JFK";
    std::string name = "Bob";
...
void testRRF()  {
    std::cout << std::endl << "\033[33;1m" << "---> Testing Robotomy Request Form <---" << "\033[37m" << std::endl;
    std::string target = "JFK";
    std::string name = "Bob";
    ...
    std::cout << "\033[33;1m" << "Testing Multiple Executions" << "\033[37m" << std::endl;
    bob_c.signForm(rrf6);
    for (int i = 0; i < 50; i++) {
       bob_c.executeForm(rrf6);
    }
